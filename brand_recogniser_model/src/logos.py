...
from brand_recogniser_model.src.utils import contents_of_bbox, features_from_image
from brand_recogniser_model.src.similarity import load_brands_compute_cutoffs, similar_matches, similarity_cutoff, draw_matches
...
def match_logo(img_test, prediction, model_preproc, outtxt, input_features_cdf_cutoff_labels,
    ...
    for idx in matches:
        ...
        labels.append((label, matches[idx][1]))
    ...
    new_img, bbox_list_list = draw_matches(img_test, input_labels, prediction, matches)
    t_draw = timer()-start
    if save_img == True:
    ...
    if timing:
        return outtxt, (t_read, t_box-t_read, t_feat-t_box, t_match-t_feat,
                        t_draw-t_match, t_save-t_draw), None

    return labels, bbox_list_list, new_img[...,::-1]
